...
async function main() {
  ...
  const nftContract:any = await NFT.deploy(_name,_symbol)
  await nftContract.deployed()

  /* deploy the marketplace */
  const Market = await ethers.getContractFactory("Marketplace")
  const marketContract:any = await Market.deploy()
  ...
  const market:any = await ethers.getContractAt("Marketplace", marketAddress)
  const nft:any = await ethers.getContractAt("BadgeToken", tokenAddress)
