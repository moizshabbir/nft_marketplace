...
// import { BadgeToken, Marketplace } from  "../typechain"
...
describe("Marketplace", function () {
  ...
  it("Should create market item with EVENT", async function() {
    await nft.mintTo(address0)  //tokenId=1
    await nft.approve(market.address,1)
    let to:any = await expect(market.createMarketItem(nft.address, 1, auctionPrice, { value: listingFee })).to;
    await to.emit(market, 'MarketItemCreated')
      .withArgs(
        1,
        nft.address,
        1,
  ...
  it("Should revert to create market item if nft is not approved", async function() {
    await nft.mintTo(address0)  //tokenId=1
    // await nft.approve(market.address,1)
    let be:any = await expect(market.createMarketItem(nft.address, 1, auctionPrice, { value: listingFee }))
      .to.be;
    await be.revertedWith('NFT must be approved to market')
  })
  ...
  it("Should create market item and buy (by address#1) successfully", async function() {
    ...
    let to: any = await expect(market.connect(account1).createMarketSale(nft.address, 1, { value: auctionPrice}))
      .to;
      await to.emit(market, 'MarketItemSold')
      .withArgs(
        1,
        nft.address,
        1,
  ...
  it("Should revert buy if seller remove approve", async function() {
    ...
    let be: any = await expect(market.connect(account1).createMarketSale(nft.address, 1, { value: auctionPrice}))
      .to.be;
    await be.reverted
  })
  ...
  it("Should revert to delete(de-list) with wrong params", async function() {
    ...
    //not a correct id
    let be: any = await expect(market.deleteMarketItem(2)).to.be;
    await be.reverted;

    //not owner
    be = await expect(market.connect(account1).deleteMarketItem(1)).to.be;
    await be.reverted;

    await nft.transferFrom(address0,address1,1)
    //not approved to market now
    be = await expect(market.deleteMarketItem(1)).to.be;
    await be.reverted;
  })
  ...
  it("Should create market item and delete(de-list) successfully", async function() {
    ...
    let be: any = await expect(market.deleteMarketItem(1))
      .to.be;
    await be.reverted;
  })
